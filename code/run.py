    ...
    for delta in range(9):
        for step_size in range(11):
            for rlength in range(3):
                print("ITER: 0"+","+str(n_directions)+","+str(delta)+","+str(step_size)+","+str(rlength))
                params['delta_std'] = math.pow(2,delta)*0.001
                params['step_size'] = math.pow(2,step_size)*0.0002
                params['deltas_used'] = int(math.pow(2,n_directions)*2)
                params['n_directions'] = int(math.pow(2,n_directions)*2)
                params['rollout_length'] = int(math.pow(2,rlength)*200)
                logz.save_params(params,"ITER: 0"+","+str(n_directions)+","+str(delta)+","+str(step_size)+","+str(rlength))
                reward = run_ars(params)
                #record
                reward_array = np.append(reward_array,reward)
                logz.save_data(reward,"ITER: 0"+","+str(n_directions)+","+str(delta)+","+str(step_size)+","+str(rlength))
                #check best
